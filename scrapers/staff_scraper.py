...
def _count_staff_in_html(html: str) -> int:
    """
    Count probable staff members on a page using two independent strategies.
    Returns the highest plausible result.
    """
    bs = BeautifulSoup(html, "lxml")
    counts: list[int] = []
...
def scrape_staff_count(website_url: str) -> int | None:
    """
    Fetch a clinic website and estimate the number of PT staff listed.
    Returns an integer count (0 = site reachable but no staff found),
    or None if the site could not be fetched at all.
    """
    if not website_url:
        return None
